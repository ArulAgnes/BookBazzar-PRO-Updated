...
-- PHP Version: 7.4.27
DROP DATABASE IF EXISTS bookbazzar;
CREATE DATABASE bookbazzar CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE bookbazzar;
...
(15, '2');
-- --------------------------------------------------------
-- NEW TABLES ADDED FOR FIXED FUNCTIONALITY
-- --------------------------------------------------------

--
-- Table structure for table `mycart` (persistent session cart)
--
...
CREATE TABLE IF NOT EXISTS `mycart` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `unique_book` (`book_isbn`,`session_id`)
...
--
-- Table structure for table `dynamic_books_table` (donation/resale uploads)
--

CREATE TABLE IF NOT EXISTS `dynamic_books_table` (
  `id` int(10) NOT NULL AUTO_INCREMENT,
...
INSERT INTO dynamic_books_table
(isbn,type,title,author,image,descr,price,publisher,category,gmail,phone)
VALUES
('D1001','Donating','C Programming','Dennis Ritchie','c.jpg','Good condition',150,'Self','Education','demo@gmail.com','9876543210'),
('D1002','Resale','Python Basics','Guido','python.jpg','Like new',250,'Self','Education','demo@gmail.com','9876501234');
...
-- --------------------------------------------------------
-- NEW (added for Accept/Reject admin workflow) --
-- Safe to run on an existing database: only adds new columns/tables.
-- --------------------------------------------------------

--
-- Seller contact info, kept on the `books` row so admin can contact
-- whoever donated/resold the book after it goes live.
--
ALTER TABLE `books`
  ADD COLUMN IF NOT EXISTS `seller_email` varchar(100) DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS `seller_phone` varchar(20) DEFAULT NULL;

--
-- Table structure for table `book_submissions`
-- Holds every book a user submits via donation.php (Donating/Resale)
-- until an admin Accepts (-> copied into `books`, visible to users)
-- or Rejects (-> stays here, hidden, for reference) it.
--
...
-- This database already ships with 2 demo rows in the old
-- `dynamic_books_table` (from donation.php's previous behaviour).
-- Migrate them across as Pending submissions so they show up ready
-- for Accept/Reject on admin_book.php out of the box:
INSERT INTO book_submissions (isbn, type, title, author, image, descr, price, publisher, category, gmail, phone, status)
SELECT isbn, type, title, author, image, descr, price, publisher, category, gmail, phone, 'Pending'
FROM dynamic_books_table;

-- `dynamic_books_table` is now legacy/unused (donation.php writes to
-- book_submissions instead) - safe to drop once you've confirmed the
-- migration above worked:
-- DROP TABLE dynamic_books_table;
